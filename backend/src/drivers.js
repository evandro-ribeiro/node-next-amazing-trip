...
exports.drivers = [
    ...
    },
    {
        ...
        "avaliacao": 4,
        "comentario": "Que viagem incrível! O carro é um show à parte e o motorista, apesar de ter uma cara de poucos amigos, foi super gente boa. Recomendo!",
        "taxa": 5.00,
        "km_minimo": 5
    },
    {
...
function getDriver(selected_km) {
    const filteredDrivers = exports.drivers.filter(driver => driver.km_minimo <= selected_km);
    if (filteredDrivers.length > 0) {
        let drivers = filteredDrivers.map(driver => {
            return {
                "id": driver.id,
                "name": driver.nome,
